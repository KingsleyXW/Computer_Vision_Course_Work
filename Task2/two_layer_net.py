...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = X.mm(W1) + b1 # broadcasting N by H
    hidden = hidden.clamp(min=0) # which is equally like relu function
...
def nn_forward_backward(
    ...
    num_train = X.shape[0]
    scores -= scores.max(axis=1).values.view(-1,1) # for numerical stability
    scores = scores.exp()
    scores_normal = scores/scores.sum(axis=1).view(-1,1)
    logyi = scores_normal[range(num_train), y]
    loss = (-logyi.log()).sum()
    loss /= num_train
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    scores_normal[range(num_train), y] -=1
    gradscores = scores_normal / num_train # here because loss/=num_train EMS
    grads['b2'] = gradscores.sum(axis=0)
    grads['W2'] = h1.t().mm(gradscores)
    grads['W2'] += reg * 2 * W2
    dh1 = gradscores.mm(W2.t())
    dh1 = dh1 * (h1 > 0)
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        for param in weights:
             params[param] -= learning_rate * grads[param] # update each layer's w
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
...
def nn_get_search_params():
    ...
    regularization_strengths = [5e-6, 1e-6, 5e-7]
        ...
        for hs in hidden_sizes:
            for rs in regularization_strengths:
                for lrd in learning_rate_decays:
                    TwoNN=TwoLayerNet(input_size, hs, output_size, device=X.device, dtype=X.dtype)
                    stats =TwoNN.train(X, y, X_val, y_val, num_iters=3000, batch_size=1000, learning_rate=lr, \
                    learning_rate_decay=lrd,reg=rs, verbose=False)
