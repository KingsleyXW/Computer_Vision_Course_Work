...
def svm_loss_vectorized(
    ...
    lables=torch.arange(0,scores.shape[1],device=W.device)
    deltas=torch.ones(scores.shape,device=W.device)
    correct_scores=deltas*scores[y,lables]
    loss_matrix=scores-correct_scores+deltas
    loss_matrix[loss_matrix<0]=0
    loss_matrix[y,lables]=0
    ...
    #############################################################################
    # Replace "pass" statement with your code
    loss_mat=scores - correct_scores + deltas 
    loss_mat[loss_mat<0]=0 # backpropagation begin
    loss_mat[loss_mat>0]=1
    loss_mat[y,lables]=0
    loss_mat[y,lables]=-1*torch.sum(loss_mat, 0)
    dW=torch.mm(loss_mat,X).t()
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    random_samples_indx = torch.randint(num_train,(1,batch_size))
    X_batch = X[random_samples_indx,:].reshape(batch_size,-1)
    y_batch = y[random_samples_indx].reshape(batch_size,)
    #########################################################################
    #                       END OF YOUR CODE                                #
    #########################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates=[1e-6, 1e-5, 5e-4, 1e-3, 0.01]
    regularization_strengths = [5e-3, 1e-2, 1e0, 1e1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
      ...
      scores_normal = scores/scores.sum()
      loss -= torch.log(scores_normal[y[i]])
      for j in range(num_classes):
        if j == y[i]:
            dW[:, j] += (scores_normal[j]*X[i] - X[i])
        else:
            dW[:, j] += (scores_normal[j]* X[i])
    loss /= num_train
    loss += reg * torch.sum(W * W)
...
def softmax_loss_vectorized(
    ...
    scores = torch.exp(scores)
    scores_normal = scores/scores.sum(axis=1).view(-1,1)
    equal_lable = scores_normal[range(num_train),y]
    loss -= torch.log(equal_lable).sum()
  
    scores_normal[range(num_train), y] -= 1 # backpropagation
    dW = X.t().mm(scores_normal)
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [5e-4, 1e-3, 5e-3, 5e-2]
    regularization_strengths = [5e-5, 5e-3, 1e-3, 1e-2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
