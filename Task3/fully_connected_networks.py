...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        x_1d = x.flatten()
        x_2d = x_1d.reshape(x.shape[0],-1) # for matrix multiplication
        dw = x_2d.t().mm(dout)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        W2 = self.params['W2']
        b2 = self.params['b2']
        hiden_score, cache_hidden1 = Linear_ReLU.forward(X, W1, b1) # comupte hidden using relu activation
        scores, cache_out  = Linear.forward(hiden_score, W2, b2) # for soft max
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dhidden, dW2, db2 = Linear.backward(dout, cache_out)
        dW2 += self.reg * W2 * 2

        dx, dW1, db1 = Linear_ReLU.backward(dhidden, cache_hidden1)
        dW1 += self.reg * W1 * 2
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
            self.params['W' + str(i)] = weight_scale * torch.randn(layer_dims[i-1], layer_dims[i], dtype=self.dtype, device=device)
            self.params['b' + str(i)] = torch.zeros((layer_dims[i],), device=device, dtype = self.dtype)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            layerscore, cache[str(i)] = Linear_ReLU.forward(layerscore, self.params['W' + str(i)], self.params['b' + str(i)])
            if self.use_dropout:
                layerscore, dropout_cache[str(i)] =Dropout.forward(layerscore, self.dropout_param)
        i = i + 1 # for last layer using relu
        layerscore , cache[str(i)] = Linear.forward(layerscore, self.params['W' + str(i)], self.params['b' + str(i)])
        scores = layerscore
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        #####################################################################
        # Replace "pass" statement with your code
        loss, upstream = softmax_loss(scores, y) # using softmax from helper
        for Wi in self.params.values():
            loss += self.reg * (Wi ** 2).sum()
        i = self.num_layers
        upstream, grads['W' + str(i)], grads['b' + str(i)] = Linear.backward(upstream, cache[str(i)]) # dx dw db

        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
               upstream = Dropout.backward(upstream, dropout_cache[str(i)])
            
            upstream, grads['W' + str(i)], grads['b' + str(i)] = Linear_ReLU.backward(upstream, cache[str(i)])
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w - lr / (torch.sqrt(config['cache']) + eps) * dw
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            # Replace "pass" statement with your code
            keep_p = 1 - p
            mask = (torch.rand_like(x) < keep_p) / keep_p
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
