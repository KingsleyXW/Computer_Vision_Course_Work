...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        Hout = 1 + (H + 2 * pad - HH) / stride # using formula
        Wout = 1 + (W + 2 * pad - WW) / stride # using formula
        Hout = int(Hout)
        Wout = int(Wout)
        ...
        x_pad = torch.nn.functional.pad(x, (pad, pad, pad, pad)) # pad last 2 d
    ...
    def backward(dout, cache):
        ...
        # dx = dx_pad[:,:,pad:-1 * pad,pad:-1 * pad]  # minus padding dimension
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        Hout = 1 + (H - pool_height) / stride # using formula
        Wout = 1 + (W - pool_width) / stride # using formula
        Hout = int(Hout)
        Wout = int(Wout)
        ...
        for n in range(N):
            for i in range(Hout):
                for j in range(Wout):
                    x_slice = x[n, :, i * stride : i * stride + pool_height, j * stride : j * stride + pool_width].reshape(C, -1)
                    out[n, :, i, j]= x_slice.max(axis = 1)[0]
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
          ...
          for c in range(C):
              for h in range(Hout):
                  for w in range(Wout):
                      ...
                      window2 = torch.reshape(window, (pool_height*pool_width, 1))
                      window3 = torch.zeros_like(window2)
                      window3[torch.argmax(window2)] = 1

                      dx[n,c,h1:h2,w1:w2] += torch.reshape(window3,(pool_height,pool_width)) * dout[n,c,h,w]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Replace "pass" statement with your code
        conv_param = {'stride': 1, 'pad': (filter_size - 1) // 2} # from loss initualization methods
        pool_param = {'pool_height': 2, 'pool_width': 2, 'stride': 2}
        C, H, W = input_dims
        HH = filter_size
        WW = HH
        conv_H = 1 + (H + 2 * conv_param['pad'] - HH) / conv_param['stride'] #  convolutional layer are choosen
        conv_W = 1 + (W + 2 * conv_param['pad'] - WW) / conv_param['stride']
        conv_H = int(conv_H)
        conv_W = int(conv_W)
        pool_H = 1 + (conv_H - pool_param['pool_height']) / pool_param['stride']
        pool_W = 1 + (conv_H - pool_param['pool_width']) / pool_param['stride']
        pool_H = int(pool_H)  # pool_H
        pool_W = int(pool_W)  # conv_H
        ...
        self.params["W2"] = weight_scale * torch.randn(
            num_filters * pool_H * pool_W, hidden_dim, dtype=dtype, device=device
        )
    ...
    def loss(self, X, y=None):
        ...
        # dx = FastMaxPool.backward(dLR, pool_param)
        # dCRP, grads['W1'], grads['b1'] = FastConv.backward(dx, CRP_cache)
    ...
    def __init__(self,
        ...
        for i,num_filter in enumerate(num_filters):
          ...
          if weight_scale == 'kaiming':
            self.params['W' + str(i)] = kaiming_initializer(prev_filters, num_filter, K = filter_size, relu = True, dtype = dtype, device = device)
          else:
        ...
        i += 1
        if weight_scale == 'kaiming':
          self.params['W' + str(i)] = kaiming_initializer(num_filter * Hout * Wout, num_classes, relu = False, dtype = dtype, device = device)
        else:
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # else:
        #   weight= weight_scale * torch.randn(Dout, Din, dtype=dtype, device=device)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            cache = (x_hat, gamma, x_mean, istd, std, var, eps, mode)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            # var = 1 / float(N) * torch.sum((x_mean) ** 2, axis=0)
            var = running_var
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x_hat, gamma, x_mean, istd, std, var, eps, mode = cache
        if mode == 'train':
            m = dout.shape[0]
...
class SpatialBatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        m = dout.permute(1,0,2,3)
        m = m.reshape(C,-1)
        m = m.t()
        dx, dgamma, dbeta = BatchNorm.backward(m, cache)
