...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        for level_name, feature_shape in dummy_out_shapes:
            if i <= 5:
                self.fpn_params[f'c{i}_p{i}'] = nn.Conv2d(feature_shape[1], self.out_channels, 1)
            else:
                break
            i += 1
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    # naive version for practice: when call nms function comment the following to use the built-in functionh above
