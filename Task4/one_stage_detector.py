...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Fill these.
        stem_cls = [nn.Conv2d(in_channels, stem_channels[0], 3, padding=1), nn.ReLU()]
        stem_box = [nn.Conv2d(in_channels, stem_channels[0], 3, padding=1), nn.ReLU()]
        # Replace "pass" statement with your code
        nn.init.normal_(stem_cls[0].weight, 0.0, 0.01)
        nn.init.normal_(stem_box[0].weight, 0.0, 0.01)
        nn.init.constant_(stem_cls[0].bias, 0.0)
        nn.init.constant_(stem_box[0].bias, 0.0)
        ...
        for i in range(len(stem_channels) - 1):
            ...
            box_block_conv, box_block_relu = nn.Conv2d(stem_channels[i], stem_channels[i+1], 3, padding=1), nn.ReLU()
            nn.init.normal_(cls_block_conv.weight, 0.0, 0.01)
            nn.init.normal_(box_block_conv.weight, 0.0, 0.01)
            nn.init.constant_(cls_block_conv.bias, 0.0)
            nn.init.constant_(box_block_conv.bias, 0.0)
            stem_cls.append(cls_block_conv)
            stem_cls.append(cls_block_relu)
            stem_box.append(box_block_conv)
            stem_box.append(box_block_relu)
        ...
        nn.init.normal_(self.pred_cls.weight, 0.0, 0.01)
        nn.init.normal_(self.pred_box.weight, 0.0, 0.01)
        nn.init.normal_(self.pred_ctr.weight, 0.0, 0.01)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feat in feats_per_fpn_level.items():
            class_level = self.stem_cls(feats_per_fpn_level[level_name])
            class_logits[level_name] = self.pred_cls(class_level)
            B,C,H,W = class_logits[level_name].shape
            class_logits[level_name] = class_logits[level_name].reshape(B,C,H*W)
            class_logits[level_name] = class_logits[level_name].permute(0,2,1)

            box_level = self.stem_box(feats_per_fpn_level[level_name])
            boxreg_deltas[level_name] = self.pred_box(box_level)
            B,C,H,W = boxreg_deltas[level_name].shape
            boxreg_deltas[level_name] = boxreg_deltas[level_name].reshape(B,C,H*W)
            boxreg_deltas[level_name] = boxreg_deltas[level_name].permute(0,2,1)

            centerness_level = self.stem_box(feats_per_fpn_level[level_name])
            centerness_logits[level_name] = self.pred_ctr(centerness_level)
            B,C,H,W = centerness_logits[level_name].shape
            centerness_logits[level_name] = centerness_logits[level_name].reshape(B,C,H*W)
            centerness_logits[level_name] = centerness_logits[level_name].permute(0,2,1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.zeros(locations.shape[0], 4).to(locations)
    if gt_boxes.shape[1] == 5:
         mask_obj = torch.where(gt_boxes[:,4] != -1)
         mask_brnd = torch.where(gt_boxes[:,4] == -1)
    else:
        mask_obj = torch.where(gt_boxes[:,:4] != torch.tensor([-1, -1, -1, -1]).to(locations))
        mask_brnd = torch.where(gt_boxes[:,:4] == torch.tensor([-1, -1, -1, -1]).to(locations))
    deltas[mask_brnd[0],:] = -torch.ones(mask_brnd[0].shape[0], 4).to(locations)
    deltas[mask_obj[0],0] = (locations[mask_obj[0],0] - gt_boxes[mask_obj[0],0])/stride
    deltas[mask_obj[0],1] = (locations[mask_obj[0],1] - gt_boxes[mask_obj[0],1])/stride
    deltas[mask_obj[0],2] = (gt_boxes[mask_obj[0],2] - locations[mask_obj[0],0])/stride
    deltas[mask_obj[0],3] = (gt_boxes[mask_obj[0],3] - locations[mask_obj[0],1])/stride
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    output_boxes = torch.zeros(deltas.shape).to(deltas)
    deltas = torch.clamp(deltas, 0)
    output_boxes = torch.zeros(locations.shape[0], 4).to(deltas)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    deltas_permute = torch.clone(deltas)
    deltas_permute[:,1] = deltas[:,2]
    deltas_permute[:,2] = deltas[:,1]
    mask_obj = torch.where(deltas!= torch.tensor([-1, -1, -1, -1], device = deltas.device, dtype = deltas.dtype))
    mask_brnd = torch.where(deltas == torch.tensor([-1, -1, -1, -1], device = deltas.device, dtype = deltas.dtype))
    if mask_brnd[0].shape[0] == 0:
       centerness = torch.sqrt(torch.min(deltas_permute[:,0:2], 1).values*torch.min(deltas_permute[:,2:], 1).values/(torch.max(deltas_permute[:,0:2], 1).values*torch.max(deltas_permute[:,2:], 1).values))
    else:
       centerness[mask_obj[0]] = torch.sqrt(torch.min(deltas_permute[mask_obj[0],0:2], 1).values*torch.min(deltas_permute[mask_obj[0],2:], 1).values/(torch.max(deltas_permute[mask_obj[0],0:2], 1).values*torch.max(deltas_permute[mask_obj[0],2:], 1).values))
       centerness[mask_brnd[0]] = -torch.ones(mask_brnd[0].shape[0], device = deltas.device, dtype = deltas.dtype)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        locations_per_fpn_level = get_fpn_location_coords(shape_per_fpn_level,self.backbone.fpn_strides, dtype=shape_per_fpn_level['p3'].dtype, device = shape_per_fpn_level['p3'].device)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        matched_gt_deltas = []
        # Replace "pass" statement with your code
        for i_gt_boxes in matched_gt_boxes:
            dictionary = {}
            for key in i_gt_boxes.keys():
                dictionary[key] = fcos_get_deltas_from_locations(locations_per_fpn_level[key], i_gt_boxes[key],  self.backbone.fpn_strides[key])
            matched_gt_deltas.append(dictionary)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        gt_classes_onehot = F.one_hot(gt_classes.long() + 1, num_classes= self.num_classes + 1)
        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=gt_classes_onehot[:,:,1:].float())
        ...
        gt_centerness = gt_centerness.reshape(pred_ctr_logits.shape)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_scores = level_pred_scores.max(dim=1)[0]
            # Step 2:
            # Replace "pass" statement with your code
            idx = (level_pred_scores > test_score_thresh).nonzero()[:,0]
            ...
            xidx = [0, 2]
            yidx = [1, 3]
            xheight = images.shape[2]
            yheight = images.shape[3]
            level_pred_boxes[:,xidx] = torch.clamp(level_pred_boxes[:,xidx],0, xheight)
            level_pred_boxes[:,yidx] = torch.clamp(level_pred_boxes[:,yidx],0, yheight)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
