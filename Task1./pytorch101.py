...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if start%10==0:
      ...
      if start-start%10+10<=stop:
        x=torch.arange(start-start%10+10,stop+1,10,dtype=torch.float64)
      else:
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    index=-(torch.arange(x.shape[0])+1)
    y=x[index,:]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def make_one_hot(x: Tensor) -> Tensor:
    ...
    idx0=torch.arange(N,dtype=torch.int64)
    idx1=torch.tensor(x)
    y[idx0,idx1]=1
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y=x[x > 0] 
    pos_sum=torch.sum(y,dtype=torch.int64).item()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return pos_sum
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y=x.view(2,3,4)
    y = y.transpose(1, 2).contiguous().view(8, 3)
    y=y.t()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    y=torch.clone(x)
    axis = -1
    min_values = y.min(dim=axis).values
    min_values_shape_corrected = min_values.unsqueeze(axis)
    y[y == min_values_shape_corrected]=0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y=torch.clone(x)
    y_column_sum=torch.sum(y,dim=0)
    y_column_mean=y_column_sum/y.shape[0]
    y_column_vsum=(y-y_column_mean)**2
    y_column_variance=torch.sum(y_column_vsum,dim=0)/(y.shape[0]-1)
    y=(y-y_column_mean)/(y_column_variance**(1/2))
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y=list(map(lambda x:float(torch.mean(x)),xs))
    y=torch.tensor(y)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    values,indice=torch.sort(x,stable=True)
    values=values.numpy()
    value=list(values)
    uniq=sorted(set(value),key=value.index)

    uni=list(uniq)
    val=value
    index=list(map(lambda x: val.index(x,0,len(val)),uni))
