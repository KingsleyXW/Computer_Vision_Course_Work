...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i,j]=(torch.sum((x_train_reshape[i,:]-x_test_reshape[j,:])**2))
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_reshape=x_test.reshape(num_test,-1)
    for i in range(num_train):
        dists[i,:]=(torch.sum(((x_test_reshape-x_train_reshape[i,:]).t())**2,dim=0))
    dists=dists.to(x_train.dtype)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_reshape=x_train.reshape(num_train,-1)
    x_test_reshape=x_test.reshape(num_test,-1)
    dists=(-2*torch.mm(x_test_reshape, x_train_reshape.t()) + (torch.sum(x_test_reshape**2,dim=1).reshape(-1,1) + torch.sum(x_train_reshape**2,dim=1)))
    dists=dists.t()
    dists=dists.to(x_train.dtype)
    ##########################################################################
    #                           END OF YOUR CODE                             #
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        k_nearest_idxs = torch.argsort(dists[:, i],descending=False)[:k]
        closest_y = y_train[k_nearest_idxs]
        y_pred[i] = torch.argmax(torch.bincount(closest_y))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        # Replace "pass" statement with your code
        num_test=x_test.shape[0]
        y_test_pred = torch.zeros(num_test, dtype=self.y_train.dtype)
...
def knn_cross_validate(
    ...
    for k in k_choices:
        for fold in range(num_folds):
           if fold==0:
              train_x=torch.cat(x_train_folds[fold+1:],0)
              train_y=torch.cat(y_train_folds[fold+1:],0)
              train_x_validation=torch.cat(x_train_folds[fold:fold+1],0)
              train_y_validation=torch.cat(y_train_folds[fold:fold+1],0)
           elif fold==num_folds-1:
              train_x=torch.cat(x_train_folds[:fold],0)
              train_y=torch.cat(y_train_folds[:fold],0)
              train_x_validation=torch.cat(x_train_folds[fold:fold+1],0)
              train_y_validation=torch.cat(y_train_folds[fold:fold+1],0)
           else:

              tr_fold1=torch.cat(x_train_folds[:fold],0)
              tr_fold2=torch.cat(x_train_folds[fold+1:],0)
              train_x=torch.cat((tr_fold1,tr_fold2),0)
            
              lb_fold1=torch.cat(y_train_folds[:fold],0)
              lb_fold2=torch.cat(y_train_folds[fold+1:],0)
              train_y=torch.cat((lb_fold1,lb_fold2),0)
              train_x_validation=torch.cat(x_train_folds[fold:fold+1],0)
              train_y_validation=torch.cat(y_train_folds[fold:fold+1],0)

           knnmodel=KnnClassifier(train_x, train_y)
            
           y_test_pred = knnmodel.predict(train_x_validation, k)
        
           num_correct = torch.sum(y_test_pred == train_y_validation)
           accuracy = 100*float(num_correct) / len(train_y_validation)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accs in sorted(k_to_accuracies.items()):
       ...
       K_mean_accuracy[k]=mean_acc

    for k, accs_m in sorted(K_mean_accuracy.items()):
       if accs_m==max(K_mean_accuracy.values()):
         best_k=k
         break
