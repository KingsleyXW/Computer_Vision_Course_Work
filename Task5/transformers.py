...
def generate_token_dict(vocab):
    ...
    idx = torch.arange(len(vocab))
    for i in range(len(vocab)):
       token_dict[vocab[i]] = idx[i].item()
...
def prepocess_input_sequence(
    ...
    for string in input_str.split(' '):
      if string.isnumeric():
         num_string = list(string)
         for num in num_string:
           out.append(token_dict[num])
      else: 
        out.append(token_dict[string])
...
def scaled_dot_product_two_loop_single(
    ...
    attn_logits = attn_logits / M**(1/2)
    attention = F.softmax(attn_logits, dim=1)
    out = torch.matmul(attention, value)
    for m in range(attention.shape[0]):
        for n in range(value.shape[1]):
            out[m,n] = (attention[m,:]).T  @ value[:,n]
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
      M = query.size()[-1]
      attn_logits = torch.matmul(query[n], key[n].transpose(-2, -1))
      attn_logits = attn_logits / M**(1/2)
      attention = F.softmax(attn_logits, dim=1)
    #   out[n,:,:] = torch.matmul(attention, value[n])
      for i in range(value.shape[2]):
         out[n,:,i] = attention @ value[n,:,i]
...
def scaled_dot_product_no_loop_batch(
    ...
    weights_softmax = torch.bmm(query, key) 
    scale = 1.0/(M)**0.5
    weights_softmax = weights_softmax * scale
    if mask is not None:
        ##########################################################################
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # weights_softmax = weights_softmax[mask] = -1e9
        mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0))
        weights_softmax = weights_softmax.masked_fill(mask == 0, -1e9) 
        # weights_softmax.div(weights_softmax.sum(2, keepdim=True)) #renormlaize
    # Replace "pass" statement with your code
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        nn.init.uniform_(self.q.weight,a=-(6/(dim_in + dim_q))**(1/2), b=(6/(dim_in + dim_q))**(1/2))
        nn.init.uniform_(self.k.weight,a=-(6/(dim_in + dim_q))**(1/2), b=(6/(dim_in + dim_q))**(1/2))
        nn.init.uniform_(self.v.weight,a=-(6/(dim_in + dim_v))**(1/2), b=(6/(dim_in + dim_v))**(1/2))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        # Replace "pass" statement with your code
        # self.d_model = dim_in
        # self.d_k = dim_in // num_heads
        self.h = num_heads
        self.heads = nn.ModuleList()
        for i in range(num_heads):
          self.heads.append(SelfAttention(dim_in, dim_out, dim_out))

        self.linearback = nn.Linear(num_heads*dim_out, dim_in)
        nn.init.uniform_(self.linearback.weight, a=-(6/(dim_in + num_heads*dim_out))**(1/2), b=(6/(dim_in + num_heads*dim_out))**(1/2))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        for head in self.heads:
           y_list.append(head.forward(query, key, value, mask))
        out = torch.cat(y_list, dim=2)
        y = self.linearback(out)
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.layer_norm = nn.LayerNorm(emb_dim, eps=self.epsilon)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.fc_1 = nn.Linear(inp_dim, hidden_dim_feedforward)
        self.fc_2 = nn.Linear(hidden_dim_feedforward, inp_dim)
        nn.init.uniform_(self.fc_1.weight,a=-(6/(inp_dim + hidden_dim_feedforward))**(1/2), b=(6/(inp_dim + hidden_dim_feedforward))**(1/2))
        nn.init.uniform_(self.fc_2.weight,a=-(6/(inp_dim + hidden_dim_feedforward))**(1/2), b=(6/(inp_dim + hidden_dim_feedforward))**(1/2))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.self_attn_layer_norm = LayerNormalization(emb_dim)
        self.dropout = nn.Dropout(dropout)
        self.positionwise_feedforward = FeedForwardBlock(emb_dim, feedforward_dim)
        self.ff_layer_norm = LayerNormalization(emb_dim)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out1= self.self_attention(x, x, x)
        #dropout, residual connection and layer norm
        src = self.self_attn_layer_norm(out1 + x)
        
        out2 = self.dropout(src)
        #positionwise feedforward
        out3 = self.positionwise_feedforward(out2)
        out4 = self.ff_layer_norm(out3 + out2)
...
def get_subsequent_mask(seq):
    ...
    mask = torch.triu(torch.ones((len_s, len_s), device=seq.device), diagonal=1).type(torch.bool)
...
class Transformer(nn.Module):
    def __init__(
        ...
        # nn.init.uniform_(self.emb_layer.weight, a=-(6/(vocab_len + emb_dim))**(1/2), b=(6/(emb_dim + vocab_len))**(1/2))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out_enc  = self.encoder(q_emb_inp)
        mask = get_subsequent_mask(ans_b[:,:-1])
        dec_out = self.decoder(a_emb_inp, out_enc, mask)
        N, O, M = dec_out.shape
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    position = torch.arange(0, K)/K
    postion = position.reshape(K,1)
    replica = torch.ones(1, M)
    y = postion @ replica
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    position = torch.arange(K).reshape(K,1).to(torch.float32)
    hiddendim = torch.arange(M).reshape(1,M).to(torch.float32)
    # torch.pow(10000, torch.floor((2 * (hiddendim) / M)))

    denom = 1/torch.pow(10000, torch.floor((2 * (hiddendim//2) / M)))
    y = position @ denom
    y[:, 0::2] = torch.sin(y[:, 0::2])  # dim 2i
    y[:, 1::2] = torch.cos(y[:, 1::2])  # dim 2i+1
    y = y.reshape(1,y.shape[0],y.shape[1])
